...
// Static mesh for the path ending puck
var private X2FadingStaticMeshComponent OutOfRangeGrapplePuck;
...
simulated event Tick(float DeltaTime)
{
	...
	// if the grapple location is not the same as the puck location and the controller is active,
	// show the out of range mesh
	if(`ISCONTROLLERACTIVE 
		&& GrappleLocations.Length > 0
		&& CursorTile != GrappleLocations[SelectedGrappleLocation].Tile)
	{
		OutOfRangeGrapplePuck.SetTranslation(CursorLocation);
		OutOfRangeGrapplePuck.SetHidden(false);
	}
	else
	{
		OutOfRangeGrapplePuck.SetHidden(true);
	}
}
...
Begin:
	...
	GrapplePuck.FadeOut();
	OutOfRangeGrapplePuck.FadeOut();
	WaypointMesh.FadeOut();
	while(GrapplePuck.IsFading() || OutOfRangeGrapplePuck.IsFading() || WaypointMesh.IsFading())
	{
		Sleep(0);
	}
...
defaultproperties
{
	...
	Begin Object Class=X2FadingStaticMeshComponent Name=OutOfRangePuckMeshComponentObject
		bOwnerNoSee=FALSE
		CastShadow=FALSE
		BlockNonZeroExtent=false
		BlockZeroExtent=false
		...
		TranslucencySortPriority=1000
	End Object
	OutOfRangeGrapplePuck=OutOfRangePuckMeshComponentObject
	Components.Add(OutOfRangePuckMeshComponentObject)
