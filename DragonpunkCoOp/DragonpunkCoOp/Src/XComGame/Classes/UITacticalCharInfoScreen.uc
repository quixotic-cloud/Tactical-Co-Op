...
simulated function PopulateCharacterData()
{
	...
	else
	{
		...
		Nickname = "\"" $ kGameStateUnit.SafeGetCharacterNickName() $ "\""; //will not be used if returned empty
		RankAbbrev = `GET_RANK_ABBRV(kGameStateUnit.GetSoldierRank(), kGameStateUnit.GetSoldierClassTemplateName());
		NameString = RankAbbrev @ kGameStateUnit.SafeGetCharacterFirstName() @ (Nickname != "\"\"" ? Nickname : "") @ kGameStateUnit.SafeGetCharacterLastName();
		...
		else
		{
			...
			ClassIconString = "img://" $ kGameStateUnit.GetMyTemplate().strTargetIconImage;
