...
class X2MPCOOPGame extends XComShell;
...
static event class<GameInfo> SetGameType(string MapName, string Options, string Portal)
{
	...
	if(InStr(GameInfoClassName, "X2MPCOOPGame", true, true) != INDEX_NONE)
	{
		return class<GameInfo>(DynamicLoadObject("Dragonpunk_CoOp.X2MPCOOPGame", class'Class')); //self.class;//GameInfoClassToUse="Dragonpunk_CoOp.X2MPCOOPGame";
	}
	else
...
function InitGameReplicationInfo()
{
	...
	kGRI = XComMPCOOPGRI(WorldInfo.GRI);
...
function UpdateGameSettingsCounts()
{
	...
	if (class'GameEngine'.static.GetOnlineSubsystem().GameInterface != None)
	{
		GameSettings = class'GameEngine'.static.GetOnlineSubsystem().GameInterface.GetGameSettings(PlayerReplicationInfoClass.default.SessionName);
		if (GameSettings != None)
		{
			...
			{
				class'GameEngine'.static.GetOnlineSubsystem().GameInterface.AddUpdateOnlineGameCompleteDelegate(m_dOnUpdateOnlineGameComplete);
				class'GameEngine'.static.GetOnlineSubsystem().GameInterface.UpdateOnlineGame('Game', GameSettings, true);
			}
		}
	}
}
...
{
	`log(self $ "::" $ GetFuncName() @ `ShowVar(SessionName) @ `ShowVar(bWasSuccessful) @ XComOnlineGameSettings(class'GameEngine'.static.GetOnlineSubsystem().GameInterface.GetGameSettings('Game')).ToString(), true, 'XCom_Online');
	class'GameEngine'.static.GetOnlineSubsystem().GameInterface.ClearUpdateOnlineGameCompleteDelegate(m_dOnUpdateOnlineGameComplete);	
...
{
	local X2MPCOOPController kLobbyPC;

	`log(self $ "::" $ GetFuncName() @ `ShowVar(m_eNextPlayerTeam), true, 'XCom_Net');
	kLobbyPC = X2MPCOOPController(Spawn(PlayerControllerClass,,, SpawnLocation, SpawnRotation,,, m_eNextPlayerTeam));
...
defaultproperties
{
	...
	m_iNumPlayers=0
	m_eNextPlayerTeam=eTeam_None
	PlayerControllerClass=class'Dragonpunk_CoOp.X2MPCOOPController'
	// we dont use the PRI/GRI classes anymore -tsmith
	PlayerReplicationInfoClass=class'Dragonpunk_CoOp.XComMPCOOPPRI'
